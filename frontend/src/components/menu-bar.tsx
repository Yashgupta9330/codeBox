...
import { Home, Settings, Bell, User, Code, Code2Icon, Sparkle } from "lucide-react"
...
export function MenuBar() {
  ...
  const isDarkTheme = state.codeTheme === "vs-dark"
    ...
    >
      ...
      <ul className="flex items-center gap-2 relative z-10">
        {menuItems.map((item, index) => (
          <motion.li key={`${item.label}-${index}`} className="relative">
            <motion.div
